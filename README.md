...
# Lens Protocol (Brownie)

The Lens Protocol is a decentralized, non-custodial social graph. 
Lens implements unique, on-chain social interaction mechanisms analogous to 
commonly understood Web2 social media interactions, but significantly expanded 
with unique functionality that empower communities to form and participants 
to own their own social graph.

This is a complete deployment implementation of official 
...
> For now only Lens Protocol deployment with partial smart contract verification has been tested 
> (i.e., LensHub and LensHubProxy smart contract verification fails on block explorer)
>
> We are now figuring out solving the same, instructions will be updated soon
>
> (feel free to experiment and submit PR's)

Create a `.env` file like `.env-example` with appropriate values. For Polygon, you need to add 
`WEB3_INFURA_PROJECT_ID` and `POLYGONSCAN_TOKEN` for deployment and verification of Lens Protocol
smart contracts.

### Local Setup

Pre-requisites
- `Python >3.8,<3.11` or `Poetry ^1.1.14`
...
Configuration
- Set the `NETWORK_ID` and `NETWORK_TYPE` in `.env`
- Provide the private keys from your blockchain wallets in `.env`


Deployment
- Run the following to start deployment
    ```
    python scripts/full_deploy.py
    ```
    or
    ```
    poetry run python scripts/full_deploy.py
    ```
